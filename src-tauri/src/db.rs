...
pub fn save_profile(conn: &Connection, profile: &UserProfile) -> Result<()> {
    conn.execute(
        "INSERT OR REPLACE INTO user_profile (id, first_name, last_name, email, address, latitude, longitude, climate_model, gemini_api_key, gemini_model, ollama_url, llm_provider)
...
pub fn get_profile(conn: &Connection) -> Result<Option<UserProfile>> {
    let mut stmt = conn.prepare(
        "SELECT first_name, last_name, email, address, latitude, longitude, climate_model, gemini_api_key, gemini_model, ollama_url, llm_provider
         FROM user_profile 
         WHERE id = 1;",
    )?;
...
/// Fetches all chat messages ordered by timestamp ascending.
pub fn get_chat_history(conn: &Connection) -> Result<Vec<ChatMessage>> {
    let mut stmt = conn.prepare(
        "SELECT id, role, content, timestamp, model_used, thoughts 
         FROM chat_messages 
...
pub fn add_chat_message(conn: &Connection, msg: &ChatMessage) -> Result<i64> {
    conn.execute(
        "INSERT INTO chat_messages (role, content, timestamp, model_used, thoughts) VALUES (?1, ?2, ?3, ?4, ?5)",
        params![msg.role, msg.content, msg.timestamp, msg.model_used, msg.thoughts],
    )?;
    Ok(conn.last_insert_rowid())
}

/// Clears all chat history from the database.
pub fn clear_chat_history(conn: &Connection) -> Result<()> {
...
mod tests {
    ...
    fn test_chat_history() {
        ...
        let history = get_chat_history(&conn).unwrap();
        assert_eq!(history.len(), 1);
        assert_eq!(history[0].role, "user");

        clear_chat_history(&conn).unwrap();
        let history_empty = get_chat_history(&conn).unwrap();
